...
namespace TheFirstPerson
{
    ...
    public class FPSController : MonoBehaviour
    {
		...
		// Goblin's custom animation stuff
		Animator anim;
		int isGrounded = Animator.StringToHash("isGrounded");
		int doubleJump = Animator.StringToHash("doubleJump");
        ...
        void Start()
        {
			// get animation controller attached to Player
        ...
        {
			// Debug.Log(""+grounded);
			// If the GameObject is jumping, send that the Boolean “grounded” is false to the Animator. 
			// The jump animation will loop while in the air.
			if (grounded == false)
			{
				anim.SetBool("isGrounded", false);
			}

			// The GameObject is grounded, so send the Boolean as enabled to the Animator. 
			// Jumping animation will stop and move to jump-windDown
			if (grounded == true)
			{
        ...
        void MouseLook()
        {
            if (mouseLookEnabled)
            {
				...
				// horizontalLook += xIn * sensitivity; the original code, to try and offset the hypersensitive turn speed on builds
                horizontalLook += xIn * sensitivity*5;
...
if (standard ? Input.GetButtonDown(jumpBtn) : customInputSystem.JumpPressed())
        ...
        {
			Debug.Log("SetData called from FPSController");
